...
\begin{abstract}
  This paper aims to develop an optimization model for allocating emergency vehicles (EVs) to fire departments. Our proposed model considers the integration dispatch of EVs in a comparatively large area. We generated some random scenarios to deal with uncertainty in the real world and conducted numerical experiments. However, the obtained solutions showed that the usefulness of our model is very limited. Therefore, we solved modifed problems in which the allocation of EVs was fixed. The results showed that our model can find the optimal or good dispatch under the fixed allocation.
\end{abstract}

\begin{keywords}
 Allocation and dispatch of EVs, integration of dispatch, optimization, uncertainty.
\end{keywords}
...
\end{quote}
When serious accidents or disasters happen, emergency vehicles (EVs) will rush to the emergency site as soon as possible for rescue. However, in recent years, the number of emergency calls is increasing. In particular, the average traveling time of EVs to the emergency site tends to become longer because of the lack of fire departments and EVs in suburbs. If we allocate more EVs in more fire departments, we can shorten the traveling time to the emergency sites. However, at the same time the allocation cost must be considered. Therefore, the white paper of the Japan Fire and Disaster Management Agency (FDMA) \mbox{\cite{FDMA18}} pointed out that we must merge the responsibility areas of fire departments and integrate the operation for dispatching EVs. This is an urgent task because we have to prepare for disaster risks and the decrease of the population of Japan in the near future.

In the allocation problem of EVs, the main objective is to minimize the response time, that is, the elapsed time between the receiving time of the emergency call and the arrival time of EVs at the emergency site \mbox{\cite{Saeed18}}. Because the emergency site would be covered by the EVs allocated at fire departments, therefore, the allocation of EVs is important for the level of service quality. Moreover, the unpredictability of the time and the location of emergency incidents are also the main issues in the EV location \mbox{\cite{keisuke04}} \mbox{\cite{Lei15}} \mbox{\cite{Xiao-Xia13}}. We should consider the uncertainty for solving the EV allocation problem:
\begin{itemize}
\item The uncertainty of the call-in time and the location of the emergency site
\item The uncertainty of the traveling time of EVs
\item The uncertainty of the service time at the emergency site
\end{itemize}

Based on the above, in this research, we develop an optimization model to find the optimal allocation of EVs with the integration of dispatching of EVs and generate some random scenarios to deal with some uncertainties which are explained above.
...
\item After call screening, dispatchers evaluate the system status and determine the appropriate EV to dispatch.
...
%\section{The probability in EV location}\label{eq:section3}
\section{Mathematical formulation}\label{section3}
In this section, we formulate the optimization model to find the optimal allocation of  EVs.
\subsection{Notation}
...
\item Indices and Sets
  ...
  \item $v \in V$ : \ the set of EVs, 
  \item $n \in N$ :\ the set of the numbers of dispatching.
  Note that each EV can be dispatched at most $|N|$ times for each scenario.
  \end{itemize}
  \item Parameters
  \begin{itemize}
...
\item
   \begin{eqnarray}
    ...
    \right.
   \end{eqnarray}
    %% $x_{a,j,n,k}$ : \ Binary decision variable that takes the value 1 if EV $k$ is dispatched to demand point $j$ in the number of dispatch $n$ under scenario $a$,and 0 otherwise.

  \item %% $y_{i,k}$ : \ Binary decision variable that takes the value 1 if EV $k$ is assigned to emergency satation $i$, and 0 otherwise.
    \begin{eqnarray}
    y_{d,v}=\left \{
    \begin{array}{ll}
  ...
  %% \item $m_{a,j,n,k}$ : \ the penalty time of the response time of an EV $k$ for a site $j$ with the $n$-th dispatch under a scenario $a$ exceeds 30 minutes.
    %$\max\{0,v_{a,n,k}-u_{a,j}\}$. ŽÔ—¼ ${k \in K}$ ‚ª $n$ ‰ñ–Ú‚Ìo“®‚ÅŒ»ê $j$ ‚Ö“ž’…‚·‚éŽž‚Æ’Ê•ñŽž‚Ì·
  %\item $q_{a,j,n,k}$ := \ $\max\{0,w_{a,n-1,k}-u_{a,j}\}$%% ŽÔ—¼ ${k \in K}$  ‚Ì $(n-1)$ ‰ñ–Ú‚ÌÁ–h‚Ö‹A’…Žž‚Æ$n$‰ñ–Ú‚ÌŒ»ê’Ê•ñŽž·
  \end{itemize}
\end{itemize}
...
\item 
 ...
 \end{description}
  The constraint (\ref{eq:Position}) ensures that the EVs are assigned to one fire department. The constraint (\ref{eq:VehicleDemand}) ensures that an EV must be dispatched to an emergency site. The constraint (\ref{eq:LimitVenue}) ensures that each EV can only be dispatched to at most one emergency site in each dispatching.
 \begin{description}
...
\item
 ...
 The constraint (\ref{eq:NeedTime}) becomes $z^\text{travel}_{s,n,v} \geq t^\text{travel}_{s,d,p_{s}}$ when $x_{s,p_{s},n,v}=1$ and $y_{d,v}=1$ hold, and $z^\text{travel}_{s,n,v}$ shows the upper bound of traveling duration between the fire department and the emergency site. The constraints (\ref{eq:GoTime}) and (\ref{eq:ArriveTime}) determine the arrival time and the end-of-service time of an EV at an emergency site, respectively. 
 The constraint (\ref{eq:BackTime}) computes the return time of an EV. The constraint (\ref{eq:BackTimeAndGoTime}) defines the relationship between the return time and the next dispatching time of an EV. The constraint (\ref{eq:Difference}) computes the delay time. Now we consider the case that $x_{s,p_{s},n,v}$ equals 1. If the response time is smaller than or equal to $t^\text{allow}$, then $z^\text{delay}_{s,p_{s},n,v}$ equals 0. Otherwise, $z^\text{delay}_{s,p_{s},n,v}$ equals the delay time, which shows the exceedance of $t^\text{allow}$.
...
\item
 ...
 \label{eq:prob2}
 \end{eqnarray}
$(\heartsuit)$ is used to find the optimal allocation of EVs by minimizing the total delay time for each EV. 
...
\item The traveling time of an EV from a fire department $d$ to an emergency site $p_{s}$ in a scenario $s$ is defined as
  \begin{center}   
    $t^\text{travel}_{s,d,p_{s}}$= $(\delta_{s,d,p_{s}}$ $/$ $q)$ $\times$ 60 + $r_{s,d,p_{s}}$ (min.),
  \end{center}
where $q$ (km/h) is the average speed of EVs and $r_{s,d,p_{s}}$ (min.) follows a uniform distribution on the interval $\left[l,u\right]$ (min.). Here, the term $r_{s,d,p_{s}}$ is included to express a potential delay.
...
In this research, we base on the above assumptions and generate the value of call-in time, traveling time and service time randomly in each scenario to deal with some uncertainties which are explained at Section \ref{section 1}.
\begin{figure}[htbp]
  \begin{center}
...
The objective value and the location of EVs are obtained by solving $(\heartsuit)$ is shown in Table~\ref{obj}. 
...
This paper focused on the EV allocation problem with integration of dispatch. The proposed model aims to minimize the delay time in order to arrive at the emergency site as fast as possible.


In order to examine the performance of the proposed model, some test instances were generated randomly and solved them. But the solver cannot ensure the accuracy of the obtained solutions: the gaps are comparatively large. However, if we fixed the allocation of EVs, then the solver can find the optimal or good dispatch. We would like to construct a new method for solving this problem by using this property in the near future. Note that: (a) we will try to find the allocation of EV by local search. (b) we will try to use the fixed allocation which was generated at phase (a) to find the optimal or good dispatch by general solver. In addition, as the future work, we also need to use the real data to examine our the performance of the proposed model. 
%% We know the model action by small-scale cases (Case (i)-1 $\sim$ Case (i)-6) by changing some parameters and use the result of the location of EVs to fix the binary variable $y_{d,v}$ (Determine the affiliation of EVs) to solve each case again. Problems are solved quickly. Next, we expanded the scale of problems (Case (ii)-1 $\sim$ Case (ii)-3). The computation results show the efficacy of the proposed model. However, if the problem scale exceeds the Case (ii)-2 like Case (ii)-3, it is hard to be solved. We plan to design a hybrid algorithm as the future work. %% 
...
Publication Pamphlet of Fire and Disaster Management Agency (in Japanese). (2015).
\url{https://www.fdma.go.jp/neuter/about/pdf/en/2015/all.pdf}, 2019/01/22.
  
\harvarditem[FDMA]{FDMA}{2018}{FDMA18}
White paper of Fire and Disaster Management Agency (in Japanese). (2018). 
\url{http://www.fdma.go.jp/html/hakusho/h29/h29/html/s4-2-2.html}, 2019/01/16.

\harvarditem[Inakawa et al.]{Inakawa et al.}{2004}{keisuke04}
Inakawa, K. and Suzuki, A. (2004).
Optimal location problem for urban EVs with continuous-time markov chain.
%% OPTIMAL LOCATION PROBLEM FOR URBAN EVS WITH CONTINUOUS-TIME MARKOV CHAIN.
